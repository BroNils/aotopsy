...
3. **Statement Tree** (`stmt.go`, `stmt_expr.go`) — emitted pseudocode is organized into a `Stmt` tree (`Line`, `Construct`, `Block`) so passes can restructure at statement/block granularity. Control-flow structure (loops, if/else, try/catch) is tree-level; most expression- and idiom-level rewrites still operate on each `Line.Text` (regex/text), so they carry the usual text-rewriting caveats and are gated conservatively to avoid fabricating semantics.
