...
// BuildX86FuncCFG builds a single-function FuncCFG for x86_64,
...
func BuildX86FuncCFG(name string, funcCode []byte, funcVA uint64, edges []disasm.CallEdge) (*FuncCFG, int) {
...
// onto FuncCFG (built for DOT rendering + call-graph accumulation)
...
func convertX86FuncCFG(fir *decompiler.FuncIR, edges []disasm.CallEdge) *FuncCFG {
	edgeByPC := make(map[uint64]disasm.CallEdge, len(edges))
	for _, e := range edges {
		edgeByPC[e.FromPC] = e
	}

	lcfg := &FuncCFG{Name: fir.Name}
	idx := 0
	for _, blk := range fir.Blocks {
		...
		for _, ins := range blk.Instrs {
			if e, ok := edgeByPC[ins.Addr]; ok {
				...
				calls = append(calls, CallSite{Offset: idx, Callee: callee})
			}
			idx++
		}
		...
		for _, s := range blk.Succs {
			lb.Succs = append(lb.Succs, Successor{BlockID: s.BlockID, Cond: s.Cond})
		}
