...
// except that MemoryMove -- its LAST leaf entry -- does not exist yet:
// runtime/vm/runtime_entry_list.h carries 35 LEAF_RUNTIME_ENTRY_LIST entries
// at tag 3.1.0 and 36 at 3.2.5, the added one being MemoryMove.
//
// Reusing the 3.2.5 list for 3.1.0 shifted no name (the extra one is last)
...
// v3.12.2: 83 RUNTIME entries from 0x2f0 to 0x588 (exclusive).
...
// v3.12.2: 48 LEAF entries stored in separate block at 0x710 (after exit_through_ffi at 0x708).
// Delta from v3.10.7: -CaseInsensitiveCompareUCS2, -CaseInsensitiveCompareUTF16,
...
// Delta from v3.10.7: -ThrowIfValueCantBeShared, +CheckedStoreIntoShared (after
// StaticFieldAccessedWithoutIsolateError), +FatalError at end.
// Source: dartsdk/v3.11.5/runtime/vm/runtime_entry_list.h:10-92
var runtimeEntriesV3115 = []string{
	// RUNTIME_ENTRY_LIST (82)
	"AllocateArray", "AllocateMint", "AllocateDouble", "AllocateFloat32x4",
	"AllocateFloat64x2", "AllocateInt32x4", "AllocateTypedData", "AllocateClosure",
...
// v3.11.5: 50 LEAF entries stored in separate block at 0x708 (after exit_through_ffi at 0x700).
// Delta from v3.10.7: TsanRead*/TsanWrite* renamed to SanRead*/SanWrite*.
// Source: dartsdk/v3.11.5/runtime/vm/runtime_entry_list.h:97-150
var leafEntriesV3115 = []string{
	"DeoptimizeCopyFrame", "DeoptimizeFillFrame",
	"StoreBufferBlockProcess", "OldMarkingStackBlockProcess", "NewMarkingStackBlockProcess",
	"EnsureRememberedAndMarkingDeferred",
	"LibcPow", "DartModulo", "LibcFmod",
...
func init() {
	...
	mergeRuntimeEntries(thrV3110, 0x2f0, runtimeEntriesV3115) // identical to v3.11.5
	mergeRuntimeEntries(thrV3110, 0x708, leafEntriesV3115)    // identical to v3.11.5
	mergeRuntimeEntries(thrV3122, 0x2f0, runtimeEntriesV3122)
	mergeRuntimeEntries(thrV3122, 0x710, leafEntriesV3122) // LEAF block after exit_through_ffi
}
...
// mergeRuntimeEntries adds runtime-entry names for offsets the SDK header
// does not export, starting at base and stepping one word per entry.
//
// The suffix is "_entry_point", matching the spelling
// runtime_offsets_extracted.h uses for the entry-point fields it DOES export
// (e.g. call_to_runtime_entry_point). It used to be "_ep": every table
// generated from the SDK then disagreed with every name synthesised here, so
// each run printed a screenful of "offset 0x2f0 has AllocateArray_entry_point,
// skipping AllocateArray_ep" -- noise that hid the real conflicts.
//
