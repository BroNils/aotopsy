...
type fridaHook struct {
	...
	// of the full raw x0-x7/rdi-r9 register set. Empty when arity wasn't
	// confidently resolved -- falls back to the full architecture default
	// (argRegsForFridaArch) rather than guessing.
	ArgRegs []string
}
...
// argRegsForFridaArch returns this.context's register names for a
// function's incoming arguments, in order, for the two architectures
// decompile-native supports. These mirror arm64ArgRegs/x86ArgRegs in
// internal/decompiler (a documented display convention, not a verified
// ABI ground truth) -- good enough for a debugging dump, not a claim
// about the real Dart AOT calling convention's exact register count.
func argRegsForFridaArch(isARM64 bool) []string {
	if isARM64 {
...
func generateFridaScriptWithOptions(libPath string, isARM64 bool, hooks []fridaHook, probes []fridaProbe, opts fridaOptions) string {
	...
	if len(probes) > 0 {
		...
		fmt.Fprintf(&b, "//   not a fluke. See generateFridaScript's doc comment in frida_gen.go.\n")
	}
	fmt.Fprintf(&b, "\n")
...
// resolved by internal/disasm.ThreadStubOffsets, nothing dynamic to
